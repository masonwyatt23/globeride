...
## Generator (historical — avatar is now procedural)

> **Note:** The `.glb` files described in this contract are no longer loaded
> at runtime. As of commit `a3dc219` the in-game avatar is built procedurally
> using Cesium primitives (`src/components/CesiumViewer.tsx`) and no longer
> reads from `public/models/`. The generator scripts (`scripts/gen-avatar-gltf.ts`
> and `scripts/gen-gear-gltf.ts`) have been removed from the repository.
>
> This contract is preserved as a reference for anyone who wants to revive
> glTF-based avatars in the future. If you do, re-implement the generators
> from scratch using the node/material/animation spec above.

---

## Gear Sub-folder (`public/models/gear/`)

Additional standalone assets that were previously generated by `scripts/gen-gear-gltf.ts`
(now removed — see generator note above):
